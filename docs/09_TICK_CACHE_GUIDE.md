...
- **Оптимизация памяти**: кэширование оптимизировано для работы с большими объемами данных (50+ миллионов тиков) без ошибок памяти
...
```
workspace/raw_data/ticks/
├── tick_metadata.pkl          # Метаданные кэша
├── XAUUSD_20240101.parquet   # Тики за 1 января 2024 (новый формат)
├── XAUUSD_20240102.parquet   # Тики за 2 января 2024 (новый формат)
└── ...
```

**Формат файлов:**
- **Parquet** (по умолчанию) - кроссплатформенный формат, не зависит от версий numpy
- **Pickle** - поддерживается для обратной совместимости, автоматически конвертируется в parquet

**Преимущества Parquet:**
- ✅ Кроссплатформенная совместимость (Windows ↔ Linux)
- ✅ Не зависит от версий numpy
- ✅ Эффективное сжатие (snappy)
- ✅ Быстрая загрузка
- ✅ Автоматическая конвертация старых pickle файлов
